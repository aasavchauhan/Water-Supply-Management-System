...
var appIds = Object.keys(ALL_APPS);
var branchAppsSchema = yupObject({
  installed: yupRecord(
...
async function sanitizeOrganizationConfig(config) {
  ...
  const products = typedFromEntries(typedEntries(prepared.payments.products).map(([key, product]) => {
    const isAddOnTo = product.isAddOnTo === false ? false : typedFromEntries(Object.keys(product.isAddOnTo).map((key2) => [key2, true]));
    const prices = product.prices === "include-by-default" ? "include-by-default" : typedFromEntries(typedEntries(product.prices).map(([key2, value]) => {
